...
For dev, DS, work, writing, workshop, and workflow-creator, native approval creates `.planning/.state/review.json` in `PENDING` state. The receipt contains exactly `workflow`, `plan_file`, `plan_hash`, `approved_session_id`, `approved_at`, `status`, `reviewer_session_id`, and `reviewed_at`. `plan_file` is the safe direct-child basename of the exact generated Markdown plan supplied by the completed native Plan interaction. `plan_hash` authenticates that file's exact bytes.
...
Never write a visible `PLAN_REVIEWED.md`, `plan.json`, or copied fixed plan for built-in workflows. External workflow descriptor schema v1 remains unchanged.
