...
```
# OR the migrated form — read-only by construction:
Workflow({ scriptPath: "<.../workflows/wc-audit.js>", args: { targetWorkflow: "{name}", projectDir: "<abs>" } })
```

### Incorrect
Main chat reads its own generated files and scores them inline without dispatching a subagent or the wc-audit workflow.
