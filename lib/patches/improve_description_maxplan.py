...
def _call_claude_cli(prompt: str, model: str) -> str:
    """Call claude -p as subprocess (works with Max plan auth)."""
...
def _make_client():
    """Create an Anthropic client if API key available, else None (will use CLI fallback)."""
    if _has_api_key():
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    if len(description) > 1024:
        ...
        if client:
            ...
            combined = f"{prompt}\n\nAssistant response:\n{text}\n\n{shorten_prompt}"
            shorten_text = _call_claude_cli(combined, model)
