...
function checkArtifactStatus(artifactPath: string, requiredStatus: string): boolean {
...
function checkRequiredFields(artifactPath: string, spec: string): string[] {
  const frontmatter = readFrontmatter(artifactPath);
...
async function main(): Promise<void> {
  ...
  const artifactPath = process.env.GATE_ARTIFACT ?? "";
  const requiredStatus = process.env.GATE_STATUS ?? "";
  const requiredFields = process.env.GATE_REQUIRE_FIELDS ?? "";
  // `??`, not `||`: Python's os.environ.get(k, default) falls back only when the var is UNSET. An
  // explicitly empty GATE_DESCRIPTION stays empty rather than becoming "Phase gate".
  const gateDescription = process.env.GATE_DESCRIPTION ?? "Phase gate";
  const gateRemedy = process.env.GATE_REMEDY ?? "Complete the previous phase first";
  const blockedToolsStr = process.env.GATE_BLOCKED_TOOLS ?? "";
  ...
  // Parse blocked tools
  const blockedTools = blockedToolsStr
    ? new Set(blockedToolsStr.split(",").map((t) => strip(t)))
  ...
  if (!blockedTools.has(toolName)) allow();

  // Allow writes to workflow state dirs (.planning/, .claude/)
  const filePath = String(toolInput.file_path ?? "");
  if (filePath && isAllowedPath(filePath)) allow();
  ...
  if (!checkArtifactExists(artifactPath)) {
    deny(
      `GATE BLOCKED: ${gateDescription} artifact missing.\n\n` +
        `Required: \`${artifactPath}\` — file does not exist.\n\n` +
  ...
  if ((requiredStatus || requiredFields) && !frontmatterIsFlat(readFrontmatter(artifactPath))) {
    deny(
      `GATE BLOCKED: ${gateDescription} — artifact is not readable.\n\n` +
        `\`${artifactPath}\` does not parse as a flat YAML mapping of simple ` +
  ...
  if (requiredStatus && !checkArtifactStatus(artifactPath, requiredStatus)) {
    deny(
      `GATE BLOCKED: ${gateDescription} — wrong status.\n\n` +
        `Required: \`${artifactPath}\` with \`status: ${requiredStatus}\`\n` +
  ...
  if (requiredFields) {
    const problems = checkRequiredFields(artifactPath, requiredFields);
    if (problems.length) {
