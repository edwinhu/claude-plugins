...
} catch { fail("ExitPlanMode approved plan path could not be recovered from transcript"); }
...
catch (error) { fail(`could not bind approved generated plan: ${error instanceof Error ? error.message : String(error)}`); }
