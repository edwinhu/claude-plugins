#!/usr/bin/env python3
"""Stage the DEF 14A filing index for the scan_covers `def14a_independence` profile.

Emits two files, because the scanner and the panel builder need different things:

  indep_filings.tsv   cik, accession, fdate, meeting_year, path
                       ...
                       `path`.
  indep_files.txt     one path per line, for `scan_covers -files-from`.
...
def main() -> None:
    ...
    idx_path = outdir / "indep_filings.tsv"
    lst_path = outdir / "indep_files.txt"
    ...
    print(f"\nNext:\n  scan_covers -profile def14a_independence -files-from {lst_path.name} \\\n"
          f"      -root /wrds/sec/wrds_clean_filings > indep_raw.tsv\n"
          f"  python build_panel.py --scan indep_raw.tsv --index {idx_path.name}")
