...
If legacy `.planning/wc/` state exists without `.planning/ACTIVE_WORKFLOW.md` declaring `workflow: workflow-creator` and `lifecycle: shared-v1`, stop. Legacy Mode 1/2/3 state, numeric steps, review markers, and HANDOFF files are not resumable or convertible. Start a fresh episode or manually align the project to the shared artifacts.
...
Enter native Plan mode only when this manifest is executable. `ExitPlanMode` persists exact approved bytes and workflow-bound metadata. Never edit `PLAN.md`, `PLAN.meta.json`, or `PLAN_REVIEWED.md` directly.
...
2. Run the read-only compiler:
   ```bash
   bun ${CLAUDE_SKILL_DIR}/../../scripts/wc/workflow-plan-compiler.ts .planning/PLAN.md --project "<absolute target root>" --json
   ```
