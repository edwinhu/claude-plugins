...
- `workflows/templates/ds-task.js` (the live per-domain FRAGMENT)
- `workflows/templates/compiled-runner-template.js` (the annotated GENERIC fragment skeleton to copy → `<domain>-task.js`)
- `scripts/ds/ds_plan_table.py` + `scripts/ds/ds_compile.py` (the splice compiler)
- `docs/DESIGN-ds-spec-plan-compile.md` (current design rationale) and `docs/DESIGN-dev-spec-plan-compile.md` (historical retired-dev design only)
...
8. **Scaffolding rules every birthed workflow inherits:** set `applies-to` on each constraint (`.md` frontmatter **and** the `.py`'s `APPLIES_TO`) — `check-all.py` honors it and otherwise runs every workflow's constraints on every project; for descriptor-v1 external workflows that still use `ACTIVE_WORKFLOW.md`, keep it at **PROJECT-ROOT/`.planning`** (check-all walks up to it); built-ins use the hidden receipt instead. Each authoring-lint must assert a **live mechanism**, not a defunct inline-string ref. A gate's `pass` derives from **ERROR-severity only** (warnings inform, never gate). Checks over **rendered output** operate on **logical units** (collapse consecutive-identical physical repeats — e.g. `#pause` animation sub-pages).
