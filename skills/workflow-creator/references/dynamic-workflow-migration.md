...
6. **Floor vs ASSIST (D1).** Deterministic probe output has two roles: a gate-bearing **floor** (→`pass`) and a verdict-feeding **assist** (a candidate-narrowing list in `evidence`+`scope.notChecked` that feeds the OUTSIDE authority but does NOT gate). Mistaking an assist for a floor and failing the gate on candidates meant for review = the **inverted-G2** false-negative defect.
