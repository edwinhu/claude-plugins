...
```bash
bun ${CLAUDE_SKILL_DIR}/../../scripts/beat/work-args.ts <abs project> --workflow writing --session ${CLAUDE_SESSION_ID}
```
...
`work.js` delegates IMPLEMENT to the `writing-draft` workflow for this domain — pass its inputs as
`domainArgs` and they are forwarded unread. A `writing-draft` failure throws rather than silently
falling back to the generic per-task path, which would build outlines/ and drafts/ by a route nobody reviewed.
