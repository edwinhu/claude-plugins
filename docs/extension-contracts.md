...
| `beat-implement-runner` | preflight request + validated WorkflowPolicy | `3` | Explicit projectDir + workflow policy + readyWave + immutable approval reset | PreflightResult with per-task approval bindings, routing decision, derived adjudication expectation, and the emitted script path when one is warranted | Thrown Error before any dispatch | Contract 2 returned per-task execution records from a Workflow script that could not run under the Workflow runtime; contract 3 splits it into this pre-step and the observation hooks, which own execution evidence. Inputs are unchanged; consumers that read result records must read hook records instead |
| `beat-spine-runner` | work.js args: projectDir + workflow + planPath + planHash + tasks, plus adapter for an external workflow | `1` | Explicit projectDir + receipt-authenticated planPath/planHash + approved task list; never discovers planning authority | { overallPass, verdict, scoreTable, implemented, verified, findings, reviews, domainRun, domainVerify, ... } — the gate computed in JS from raw counts | Thrown Error before any agent is dispatched | A BUILT-IN workflow is adapter-driven from the internal table and a caller-supplied adapter is refused, so a caller cannot choose who audits it. An EXTERNAL workflow must supply args.adapter and its shape is validated. This is the surface an external plugin uses to run the shared IMPLEMENT/VERIFY/REVIEW beats; beat-implement-runner remains the contract-3 pre-step for consumers that dispatch the wave themselves |
| `beat-spine-args` | CLI: <projectDir> --workflow <name> [--session <id>] | `1` | Explicit projectDir + workflow identity + current session | One JSON object { projectDir, workflow, planPath, planHash } re-hashed against the plan's current bytes | Non-zero exit with a [work-args] diagnostic naming the receipt state — missing-artifact, review-pending, stale-receipt, or a receipt identity disagreement | Emits the two authority fields only; tasks are never derived from plan prose, because TaskList is not readable from a script and inventing them would be the LLM-discovery fallback this lifecycle refuses |
