...
**The reviewer identity is derived, never supplied.** `finalizeComposedPlanReview` takes `reviewerAuthorizationNonce`, not a reviewer id. Earlier revisions took `reviewerSessionId: string`, and the approver-must-differ-from-reviewer rule then compared the approver to whatever literal the caller passed — so a single actor could finalize an APPROVED receipt naming a reviewer that never existed, and admission accepted it. The nonce is issued by `reviewer-verdict-guard` on the mandatory pre-finalization plan rehash, for the actor the hook payload actually names (`hookActorIdentity`), and is delivered back through `additionalContext`. It is written to `.planning/.state/reviewer-authorization.json`, bound to the workflow and plan hash, and is single use: finalization consumes it, and re-binding an approval destroys any leftover. A caller that supplies an identity in either parameter position gets `reviewer-authorization`.
