...
| **workflow-creator** | `wc-generate.js` (+ `workflow-creator-verify.js`); file generation | structural / test | ◑ low priority | Meta-tooling; `wc-generate` already works. Port only if it earns its keep; not load-bearing. |
