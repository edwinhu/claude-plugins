...
- **P22 Compile-vs-interpret fit** — is the work-list deterministically COMPILED (parser → run.js OR data index), NOT re-discovered by an in-workflow LLM agent each call? A generic-interpreter scores LOW; a data-variant with a deterministic compile scores HIGH (do NOT penalize the absence of run.js). For a CODE-variant (post pass #9), does the compiler **splice the shared `run-core.js`** (one driver) rather than carry a hand-copied parallel driver body (a duplicate driver is drift-risk — deduct)?
