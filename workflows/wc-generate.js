...
const KIND_GUIDE = {
  ...
  'constraint-md': 'a constraint/convention rule file: frontmatter (name, description, applies-to), Rule, Rationale (cite the real failure mode from DESIGN), Examples (correct/incorrect), optional Facts section (ONLY incident-grounded non-derivable facts — numbers, thresholds, named incidents, tool quirks — each with its consequence stated as a property of the action; NO excuse/reality Rationalization Tables and NO standalone Drive-Aligned tables, both deprecated v5.36.0), Red Flags (action-targeted "About to X -> STOP" one-liners only).',
