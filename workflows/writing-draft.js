...
const VERIFY_SCHEMA = {
  ...
  properties: {
    ...
    coverageOk: { type: 'boolean', description: 'is EVERY outline point + evidence item expanded in the prose (no skipped subsections, not cursory)?' },
...
for (const s of draftable) {
  ...
  tasks.push(() => agent(
    `You are a writing-draft prose generator. You EXPAND one section outline into prose — the outline is a CONTRACT and the "what" is pinned. Your latitude is LOCAL only: sentence craft, word choice, footnote formatting, and the transition bridges to neighbors. You add NO new claims, NO structure the outline didn't specify, NO citations the outline didn't pin.
...
- EVERY outline point → at least one paragraph. EVERY subsection transition → an explicit bridge. Match the outline's word-count/paragraph target — a 2-paragraph section under a 5-subsection outline is a STUB, not a draft.
...
1. coverageOk — is EVERY outline point AND every evidence item expanded into prose? A skipped subsection or a cursory 2-paragraph treatment of a 5-point outline ⇒ coverageOk=false (major; a wholly missing subsection is critical).
