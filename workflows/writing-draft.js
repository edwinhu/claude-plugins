...
- HEADINGS — outline scaffolding is NOT document headings. The outline's \`## Opening\`, \`## Body\`, and \`## Closing\` are SCAFFOLDING LABELS that organize the outline; NEVER emit them — or any synonym like "Conclusion to Part II" / "Closing" — as a heading in the prose. The ONLY headings in your output are the section TITLE (\`# <Section Name>\`) and the lettered subsection headings (\`## A. <Name>\`, \`## B. <Name>\`, …). Concretely: "Opening" → the section's lead paragraph(s) with NO heading; "Body" → the lettered subsections, which DO take \`## A./B./C.\` headings; "Closing" → a trailing UNHEADED bridging paragraph after the last lettered subsection. (Opening already works this way; Closing must too — a Part ends in an unheaded bridge, never a "Closing"/"Conclusion" heading.)
