...
describe("workflow-creator pre-plan admission", () => {
  test("admits exact read-only wc-audit", () => expect(run({ name: "wc-audit", args: validArgs }).stdout).toBe(""));
  test("denies other pre-plan workflows", () => expect(run({ name: "wc-generate", args: validArgs }).stdout).toContain('"permissionDecision": "deny"'));
  test("denies malformed or mutation-bearing wc-audit", () => {
    expect(run({ name: "wc-audit", args: { ...validArgs, readOnly: false } }).stdout).toContain('"permissionDecision": "deny"');
    expect(run({ name: "wc-audit", args: { ...validArgs, writablePaths: ["skills/demo/SKILL.md"] } }).stdout).toContain('"permissionDecision": "deny"');
    expect(run({ name: "wc-audit", args: { ...validArgs, mechanicalProbes: [{ command: "rm -rf /tmp/demo" }] } }).stdout).toContain('"permissionDecision": "deny"');
  });
  test("audit-only agents have a structural read-only tool profile", () => {
    ...
    const workflow = readFileSync(join(root, "workflows", "wc-audit.js"), "utf8");
