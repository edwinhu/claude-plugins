...
- **13F filings declared `windows-1252` parsed to ZERO holdings rows — 7,023 filings and 2,628,463 rows recovered.** `encoding/xml` refuses a non-UTF-8 declaration when `CharsetReader` is nil; `parseInfoTable` swallowed the error and returned no rows while the filing still recorded `parse_status=ok`, `parse_mode=xml`, `n_rows=0`. This is the failure mode nothing downstream can see: a filing that parses to zero rows looks exactly like an institution that did not file.
  - **Blast radius, measured** by re-parsing the exact 7,678 filings that produced zero rows under the shipped parser: **7,023 recovered** (2.83% of the corpus), **+2,628,463 holdings rows** (+3.04% on 86,444,026), **+$25.74tn of $670.71tn reported value** (+3.84%), **768 distinct institutions**. All 38 quarters affected. The remaining 655 are genuinely empty tables; no filing hit a charset we cannot decode.
  - **Skewed to large foreign banks, asset managers and public pensions** — RBC ($6.7tn, 402,830 rows), Barclays, CalPERS, DZ Bank, Schroder, Nomura, Caisse de Dépôt, Two Sigma, Farallon, MetLife, Saudi PIF. **Zero index managers affected** (no BlackRock, Vanguard, State Street, Geode, Northern Trust, Fidelity, Dimensional, Invesco, Schwab) — which makes it worse, not better: the loss fell entirely on the non-index side, shrinking the denominator and **inflating `index_pct` and `passive_pct`**.
  - **The bias is time-varying**, which is the dangerous part. Recovered filings run ~85–130/quarter through 2023Q2, jump to 213 in 2023Q3, then hold near 400; distinct affected filers go 157 → 519. The break is a filing-agent effect (agents `0001140361`, `0000905148`, `0000945621` show `pre=0, post=N`), not a filer one. A step change in dropped non-index ownership at 2023Q3 **masquerades as a rise in passive share**.
